...
// All 10 rule modules wired and live:
...
async function check({ amountWei, payTo, agentAddress, resource, sessionId, vaultAddress }) {
  ...
  if (spendTokens > 500) flags.push({ level: 'CRITICAL', reason: `24h spending exceeds 500 tokens` });
  else if (spendTokens > 100) flags.push({ level: 'HIGH', reason: `24h spending exceeds 100 tokens` });
  else if (spendTokens > 50) flags.push({ level: 'MEDIUM', reason: `24h spending exceeds 50 tokens` });
  ...
  try {
    ...
    const isKnownService = catalogServices.some(s => {
      // Match by payTo address
      if (s.payTo?.toLowerCase() === payTo.toLowerCase()) return true;
      // Match by hostname
      try {
        return new URL(s.resource).hostname === new URL(resource).hostname;
      } catch {
        return false;
      }
    });
  ...
  try {
    ...
    if (catalogEntry && catalogEntry.payTo?.toLowerCase() !== payTo.toLowerCase()) {
      flags.push({
        level: 'CRITICAL',
        reason: `payTo mismatch: catalog lists ${catalogEntry.payTo.slice(0,8)}... for this service, but agent pays ${payTo.slice(0,8)}...`
      });
    }
  } catch { /* hostname parse fail caught by input validation */ }
  ...
  try {
    ...
    for (const s of catalogServices) {
      ...
      if (dist > 0 && dist <= 2 && host !== catHost) {
        flags.push({
          level: 'HIGH',
          reason: `URL closely resembles known service "${catHost}" — possible typosquatting`
        });
        break;
      }
    }
  ...
  if (dupRow.c >= 2) {
    flags.push({
      level: 'MEDIUM',
      reason: `Duplicate payment: same recipient, amount, and service paid ${dupRow.c} times in last hour`
    });
  }
  ...
  if (sessionId || vaultAddress) {
    try {
      const contextFlags = await checkContextAnomaly({
        sessionId, vaultAddress, amountWei, resource
      });
  ...
  try {
    ...
    if (hasUrgency) {
      // Escalate based on current max severity
      const currentMax = flags.reduce(
        (max, f) => Math.max(max, SEVERITY_VALUE[f.level] ?? 0), 0
      );
  ...
  if (selfTargets.includes(payTo.toLowerCase())) {
    flags.push({
      level: 'HIGH',
      reason: 'Payment targets the agent itself, its vault, or a core settlement contract'
    });
  }
  ...
  return { level: finalLevel, flags, trustTier, threatIntel };
}
